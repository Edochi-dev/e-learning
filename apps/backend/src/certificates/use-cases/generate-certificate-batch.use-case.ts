...
export class GenerateCertificateBatchUseCase {
    ...
    async execute(dto: GenerateCertificateBatchDto): Promise<GeneratedCertificateSummary[]> {
        const template = await this.templateGateway.findOne(dto.templateId);
        if (!template) throw new NotFoundException(`Plantilla ${dto.templateId} no encontrada`);
        ...
        const templateAbsPath = join(this.publicDir, template.filePath.replace('/static/', ''));
        ...
        for (const name of dto.names) {
            ...
            const count = await this.certificateGateway.countByAbbreviation(template.courseAbbreviation);
